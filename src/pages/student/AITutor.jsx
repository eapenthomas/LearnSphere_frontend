...
const AITutor = () => {
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}/api/ai-tutor/upload`, {
        method: 'POST',
        body: formData,
      });
  ...
  const handleAskQuestion = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}/api/ai-tutor/ask`, {
        method: 'POST',
        headers: {
