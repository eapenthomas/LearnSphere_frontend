...
const QuizResult = () => {
  ...
  const fetchQuizAndSubmission = async () => {
    try {
      ...
      // Fetch quiz details
      const quizResponse = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}/api/quizzes/${quizId}`);
      ...
      const submissionResponse = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}/api/quizzes/${quizId}/submissions`);
