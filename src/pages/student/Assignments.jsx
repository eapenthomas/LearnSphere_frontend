...
const StudentAssignments = () => {
  ...
  const handleSubmitAssignment = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || '${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}'}/api/assignments/submit`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${user.accessToken}`
        },
  ...
  const handleDownloadAssignment = async (assignmentId) => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || '${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}'}/api/files/download/assignment/${assignmentId}`, {
        headers: {
          'Authorization': `Bearer ${user.accessToken}`
        }
      });
