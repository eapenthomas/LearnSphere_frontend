...
const StudentAllCourses = () => {
  ...
  const fetchAllCourses = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}/api/student/courses`, {
        headers: {
  ...
  const handleEnroll = async (course) => {
    ...
    try {
      ...
      if (course.is_paid && course.enrollment_status === 'not_enrolled') {
        ...
        const paymentResponse = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}/api/student/initiate-payment/${courseId}`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${user.accessToken}`
          }
        ...
        const options = {
          ...
          handler: async function (response) {
            // Verify payment
            const verifyResponse = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}/api/student/verify-payment`, {
              method: 'POST',
              headers: {
        ...
        const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}/api/enrollments/enroll`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${user.accessToken}`
  ...
  const handleUnenroll = async (courseId) => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}/api/enrollments/student/${user.id}/course/${courseId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        }
