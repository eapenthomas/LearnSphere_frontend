...
const AIUsage = () => {
  ...
  const fetchAIUsageData = async () => {
    try {
      ...
      const statsResponse = await fetch('${import.meta.env.VITE_API_BASE_URL || '${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}'}/api/admin/ai-usage/stats');
      if (statsResponse.ok) {
      ...
      // Fetch recent usage
      const recentResponse = await fetch('${import.meta.env.VITE_API_BASE_URL || '${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}'}/api/admin/ai-usage/recent?limit=10');
      ...
      const providerResponse = await fetch('${import.meta.env.VITE_API_BASE_URL || '${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}'}/api/admin/ai-usage/providers');
