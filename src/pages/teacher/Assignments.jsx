...
const TeacherAssignments = () => {
  ...
  const fetchAssignments = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || '${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}'}/api/assignments/teacher/${user.id}`);
  ...
  const fetchSubmissions = async (assignmentId) => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || '${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}'}/api/assignments/submissions/${assignmentId}?teacher_id=${user.id}`);
  ...
  const handleCreateAssignment = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || '${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}'}/api/assignments/create?teacher_id=${user.id}`, {
        method: 'POST',
        body: formDataToSend
      });
  ...
  const handleGradeSubmission = async (submissionId, score, feedback = '') => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || '${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}'}/api/assignments/grade/${submissionId}?teacher_id=${user.id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleDownloadFile = async (fileType, fileId) => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || '${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}'}/api/files/download/${fileType}/${fileId}`, {
        headers: {
