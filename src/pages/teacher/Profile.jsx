...
const TeacherProfile = () => {
  ...
  const fetchProfileData = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}/api/teacher/reports/profile/${user.id}`);
      if (response.ok) {
  ...
  const fetchTeacherStats = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}/api/teacher/reports/stats/${user.id}`);
      if (response.ok) {
        const data = await response.json();
        setStats({
  ...
  const fetchTeacherRatings = async () => {
    try {
      ...
      const summaryResponse = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}/api/teacher-ratings/teacher/${user.id}/summary`);
      ...
      const ratingsResponse = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}/api/teacher-ratings/teacher/${user.id}?limit=5`);
  ...
  const handleSaveSettings = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}/api/teacher/settings/${user.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
