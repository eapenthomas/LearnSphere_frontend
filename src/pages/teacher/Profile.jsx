...
const TeacherProfile = () => {
  ...
  const fetchProfileData = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/teacher/reports/profile/${user.id}`);
      if (response.ok) {
  ...
  const fetchTeacherStats = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/teacher/reports/stats/${user.id}`);
      if (response.ok) {
        const data = await response.json();
        setStats({
  ...
  const fetchTeacherRatings = async () => {
    try {
      ...
      const summaryResponse = await fetch(`${API_BASE_URL}/api/teacher-ratings/teacher/${user.id}/summary`);
      ...
      const ratingsResponse = await fetch(`${API_BASE_URL}/api/teacher-ratings/teacher/${user.id}?limit=5`);
  ...
  const handleSaveSettings = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/api/teacher/settings/${user.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
