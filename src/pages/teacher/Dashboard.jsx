...
const TeacherDashboard = () => {
    ...
    try {
      ...
      for (const course of courses) {
        try {
          ...
          const enrollmentsResponse = await fetch(`${import.meta.env.VITE_API_BASE_URL || '${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}'}/api/enrollments/course/${course.id}`);
      ...
      for (const assignment of assignments) {
        const submissionsResponse = await fetch(`${import.meta.env.VITE_API_BASE_URL || '${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}'}/api/assignments/submissions/${assignment.id}?teacher_id=${user.id}`);
  ...
  const fetchAnalyticsData = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || '${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}'}/api/teacher/analytics/${user.id}`);
      if (response.ok) {
