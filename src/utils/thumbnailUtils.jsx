...
export const getThumbnailUrl = (course) => {
  ...
  if (course.thumbnail_url) {
    ...
    if (isSupabaseStorageUrl(course.thumbnail_url)) {
      return `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}/api/thumbnails/course/${course.id}`;
    } else {
...
export const convertToBackendUrl = (supabaseUrl, resourceId, resourceType) => {
  if (isSupabaseStorageUrl(supabaseUrl)) {
    return `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}/api/thumbnails/${resourceType}/${resourceId}`;
  }
  return supabaseUrl;
};
