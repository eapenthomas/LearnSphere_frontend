...
const EnhancedCourseViewModal = ({ isOpen, onClose, course }) => {
  ...
  const fetchCourseProgress = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/progress/course/${course.id}/student/${user.id}`);
      if (response.ok) {
  ...
  const trackMaterialView = async (material) => {
    ...
    try {
      ...
      await fetch(`${API_BASE_URL}/api/progress/material/track?student_id=${user.id}&course_id=${course.id}`, {
        method: 'POST',
        headers: {
  ...
  const trackMaterialDownload = async (material) => {
    ...
    try {
      ...
      await fetch(`${API_BASE_URL}/api/progress/material/download?student_id=${user.id}&course_id=${course.id}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
                        ...
                        <div className="flex items-center space-x-2">
                          <button
                            onClick={() => {
                              ...
                              window.open(`${API_BASE_URL}/api/files/view/material/${material.id}`, '_blank');
                            }}
