...
const QuizGenerator = ({ courseId, teacherId, onQuizGenerated }) => {
  ...
  const handleGenerateQuiz = async () => {
    ...
    try {
      ...
      const response = await fetch('${import.meta.env.VITE_API_BASE_URL || '${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}'}/api/quiz-generator/generate', {
        method: 'POST',
        body: formData
      });
  ...
  const handleSaveQuiz = async (editedQuiz) => {
    setSaving(true);
    try {
      const response = await fetch('${import.meta.env.VITE_API_BASE_URL || '${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}'}/api/quiz-generator/save-generated-quiz', {
        method: 'POST',
        headers: {
